...
## [2026-08-29] service | 말랑말랑팡팡 — 사운드 전면 도입, 두 번째 맵(산책)
효과음 Kenney(CC0) · BGM 魔王魂(무료 상업이용, 크레딧 필수 — 설정 패널에 표기).
**iOS는 Ogg Vorbis를 디코딩하지 못한다**는 걸 기기 로그로 발견 — 캐싱은 조용히 성공하고
재생 순간에만 실패해서 "소리가 없다"로 보였음. 전량 AAC 변환. 이어서 `click_003`(7ms),
`tick_001`(20ms)은 원본부터 너무 짧아 들리지 않아 교체. 버튼 하나당 소리 하나로 정리
(버스 클릭음 + 핸들러 전환음이 겹쳐 있었음). 콤보는 `bong_001`을 온음 간격 8단계로
오프라인 프리렌더 — 런타임 피치 API가 플랫폼마다 동작이 달라 신뢰 불가.

두 번째 맵 `StrollScene` 추가: 탭한 곳으로 캐릭터가 걸어가고, 지나가면 젤리가 터짐.
1스테이지 클리어 → 2스테이지로 이어지며 점수는 `GameSession.carry()` 로 넘김.
캐릭터는 CraftPix 무료 팩 Schoolgirl_2 (상업 이용 가능, 출처 표기 불필요).
스프라이트 시트는 공용 59×57 사각형으로 크롭 — 원본은 128×128 프레임에 34×45만 그려져
있어 작게, 7px 왼쪽으로 치우쳐 렌더됐음.

시뮬레이터 스크린샷이 테스트가 못 잡은 크래시 두 건을 잡음: 카메라 우선순위 문제로
모달이 월드를 못 덮던 것, `OpacityEffect`가 `OpacityProvider`를 요구해 첫 젤리 수집에서
터지던 것. 커밋: 9a07d15, 2967ded, 그리고 이 배치.
